<a href="{{ route('hasilproduksi.index', ['key' => 'unduh']) }}&tipe=marinasi&tanggal={{ $tanggal }}" class="btn btn-success">Unduh</a>

<div class="table-responsive">
    <table width="100%" id="kategori" class="table default-table dataTable-marinasi">
        <thead>
            <tr>
                <th>No</th>
                <th>Nama</th>
        ...
        <tbody>
            @foreach ($marinasi as $i => $part)
                <tr>
                    ...
                    <td>{{ number_format($part->qty_item, 0) }}</td>
                    <td>{{ number_format($part->stock_berat, 2) }}</td>
                    <td>{{ number_format($part->stock_item, 0) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
@section('footer')
    ...
    <script>
        $(document).ready(function() {
            if ($.fn.DataTable.isDataTable('.dataTable-marinasi')) {
                $('.dataTable-marinasi').DataTable().destroy();
            }
            $('.dataTable-marinasi').DataTable({
                "bPaginate": true,
                "bLengthChange": false,
                "bFilter": true,
                "bInfo": false,
