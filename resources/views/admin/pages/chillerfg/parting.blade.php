<a href="{{ route('hasilproduksi.index', ['key' => 'unduh']) }}&tipe=parting&tanggal={{ $tanggal }}" class="btn btn-success">Unduh</a>

<div class="table-responsive">
    <table width="100%" id="kategori" class="table default-table dataTable-parting">
        <thead>
            <tr>
                <th>No</th>
                <th>Nama</th>
...
@section('footer')
    ...
    <script>
        $(document).ready(function() {
            if ($.fn.DataTable.isDataTable('.dataTable-parting')) {
                $('.dataTable-parting').DataTable().destroy();
            }
            $('.dataTable-parting').DataTable({
                "bPaginate": true,
                "bLengthChange": false,
                "bFilter": true,
                "bInfo": false,
