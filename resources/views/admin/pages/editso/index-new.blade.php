                ...
                @foreach ($store as $i => $item)
                    ...
                    <tr>
                        <td>
                            ...
                            @endif
                            @if ($item->historyDeleteBB > 0)
                                <br>
                                <a href="{{ url('admin/editso/'.$item->id.'/riwayat')}}" class="btn btn-primary btn-sm historyDeleteBB mt-2" data-toggle="modal" data-target="#riwayat" data-id="{{$item->id}}">
                                    History Delete
                                </a>
                            @endif
                        </td>
                        <td>
                            <span class="status status-info">{{ number_format($item->qty ?? '0') }} || {{ number_format($item->berat ?? '0', 2) }} kg</span>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            @else
                                <button type="button" class="btn btn-default mt-1" data-toggle="modal" data-target="#exampleModal" id="form-item-name{{ $item->id }}" onclick="return selected_id('{{ $order->id }}','{{ $item->id }}','{{ $item->item_id }}')">
                                    Tambah Item 
                        ...
                        <td>
                            @if ($item->historyreset > 0)
                                <a href="{{ url('admin/editso/'.$item->id.'/riwayat')}}" class="btn btn-primary btn-sm history_reset" data-toggle="modal" data-target="#riwayat" data-id="{{$item->id}}">History</a>
                            @endif
...
<section class="panel">
    <div class="card-body">
        ...
        @php
            $ceklog = App\Models\Adminedit::where('table_id', $order->id)->where('table_name', 'orders')->where('type','edit')->count();
        @endphp
        @if ($ceklog > 0)
        <a href="{{ url('admin/editso/'.$order->id.'/riwayat')}}" class="btn btn-primary btn-sm riwayat_edit_sodo" data-toggle="modal" data-target="#riwayat" data-id="{{$order->id}}" style="float: right;">
            history edit
        </a>
...
<script>
    ...
    function loadIntegrasiNetsuite(id){
        var uri = "{{ route('editso.getintegrasinetsuite') }}" + "?id=" + id;
        $.ajax({
            url         : uri,
            method      : "GET",
            beforeSend  : function(){
