@php
    ...
    $do     =   [];
@endphp
@if($pemenuhan->bahan_baku)
    @foreach ($pemenuhan->bahan_baku as $no => $row)
        @php
            $pcs    +=  $row->bb_item ;
            $kg     +=  $row->bb_berat ;
            $krj    +=  $row->keranjang ;
        ...
        <div class="border-bottom">
            ...
            @if ($row->relasi_netsuite_one)||
            <a href="{{url('admin/sync-detail/'.$row->netsuite_id)}}" target="_blank"><span class="green">{{$row->relasi_netsuite_one->document_no ?? "#TI"}}</span></a> || @endif  
