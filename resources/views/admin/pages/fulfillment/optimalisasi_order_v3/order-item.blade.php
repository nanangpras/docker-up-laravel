...
@foreach ($store as $no => $row)
    ...
    <div class="border-bottom">
        ...
        @if ($nomorTI)||
        <a href="{{url('admin/sync-detail/'.$row->netsuite_id)}}" target="_blank"><span class="green">{{$nomorTI->document_no ?? "#TI"}}</span></a> || @endif  
        ...
        @if ($row->status == 1 && $row->statusbahanbborder <= 6)
