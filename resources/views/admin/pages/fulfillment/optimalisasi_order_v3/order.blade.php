            ...
            <tbody>
                ...
                @foreach ($masterData as $item)
                    @if($val->id == $item->order_id)
                        ...
                        @if(in_array((string)$item->category_id, (json_decode(json_encode($inkategori)))))
                        <tr>
                            <td>
                                {{ $item->nama_detail }}
                                <br/>
...
<script>
    ...
    function loadIntegrasiNetsuite(id){
        ...
        var uri = "{{ route('fulfillment.order') }}" + "?id=" + id + "&jenis=" + jenis + "&divisi=" + divisi+"&key=integrasinetsuite";
        $.ajax({
            url         : uri,
            method      : "GET",
            beforeSend  : function(){
