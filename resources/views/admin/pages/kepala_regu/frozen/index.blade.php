...
@section('content')
    ...
    <div class="row mb-4">
        ...
        <div class="col text-center">
            <b>Preparation Kepala Regu Frozen</b>
        </div>
        <div class="col"></div>
    </div>
    ...
    <script>
        $("#show_data").load("{{ route('kepalaregu.frozenshow') }}")
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.selesai_freestock', function() {
                ...
                $.ajax({
                    url: "{{ route('kepalaregu.frozenfreestockselesai') }}",
                    method: "POST",
                    data: {
                        row_id: row_id,
                    },
                    success: function(data) {
                        $("#freestock").load("{{ route('kepalaregu.frozenfreestock') }}");
                    }
                });
            })
            ...
            $(document).on('click', '.input_freestock', function() {
                ...
                $.ajax({
                    url: "{{ route('kepalaregu.frozenfreestockstore') }}",
                    method: "POST",
                    data: {
                        row_id: row_id,
                        freestock: freestock,
                    ...
                    success: function(data) {
                        ...
                        $.get("{{ route('kepalaregu.frozenfreestock') }}", { 'id' : row_id }, function(data) {
                            $('#tempoarary' + row_id).html(data) ;
                        });

                        $.get("{{ route('kepalaregu.frozenfreestock') }}", { 'list' : row_id }, function(data) {
                            $('#liststock' + row_id).html(data) ;
                        });

                        $('input:radio[name=ukuran_ayam' + row_id + ']:checked')[0].checked = false;
                        $("#item" + row_id).val('');
                        $("#qty" + row_id).val('');
                    }
                });
            ...
            $(document).on('click', '.del_temporary', function() {
                ...
                $.ajax({
                    url: "{{ route('kepalaregu.frozenfreestockdelete') }}",
                    method: "POST",
                    data: {
                        row_id: row_id,
                        item: item,
