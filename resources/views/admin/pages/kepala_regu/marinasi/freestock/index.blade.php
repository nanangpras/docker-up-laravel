    ...
    @foreach ($fresh as $row)
                                ...
                                <div class="row mb-2">
                                    <div class="col-9">
                                        <select name="item" class="form-control select2" id="item{{ $row->id }}">
                                            <option value="" disabled selected hidden>Pilih Produk Marinasi</option>
        ...
        <script>
            $("#liststock{{ $row->id }}").load(
                "{{ route('kepalaregu.marinasifreestock', ['list' => $row->id]) }}");
            $("#tempoarary{{ $row->id }}").load(
                "{{ route('kepalaregu.marinasifreestock', ['id' => $row->id]) }}");
