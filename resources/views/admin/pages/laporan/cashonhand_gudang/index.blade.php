...
@section('title', 'Cash on Hand Gudang')

@section('footer')
<script>
    $("#data_view").load("{{ route('dashboard.cashonhandgudang', ['key' => 'view']) }}&regu={{ $request->regu }}&tanggal_awal={{ $request->tanggal_awal }}&tanggal_akhir={{ $request->tanggal_akhir }}")

    $("#cari_data").on('keyup', function() {
        var cari    =   $(this).val() ;
        $("#data_view").load("{{ route('dashboard.cashonhandgudang', ['key' => 'view']) }}&regu={{ $request->regu }}&tanggal_awal={{ $request->tanggal_awal }}&tanggal_akhir={{ $request->tanggal_akhir }}&page=1&cari=" + cari);
    });
</script>
@endsection
