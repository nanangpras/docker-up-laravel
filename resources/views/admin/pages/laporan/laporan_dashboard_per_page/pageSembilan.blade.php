<div class="row mb-3">
    <div class="col-md-6">
        <div class="font-weight-bold">Summary Bahan Baku Parting</div>
            ...
            <table class="table default-table">
                ...
                <tbody>
                    @php
                        ...
                        $berat = 0;
                    @endphp
                    @foreach ($produksi['bb_parting'] as $row)
                        @php
                            $ekor += $row->total;
                            $berat += $row->kg;
                        @endphp
                ...
                <tfoot>
                    <tr>
                        <th colspan="3">Total Bahan Baku Parting</th>
    ...
    <div class="col-md-6">
        <div class="font-weight-bold">Summary Produksi Parting</div>
        <div class="form-group outer-table-scroll">
            <table class="table default-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @php
                        ...
                        $berat = 0;
                    @endphp
                    @foreach ($produksi['fg_parting'] as $row)
                        @php
                            $ekor += $row->total;
                            $berat += $row->kg;
                        @endphp
                ...
                <tfoot>
                    <tr>
                        <th>Total Hasil Produksi Parting</th>
    ...
    <div class="col-md-12">
        <div class="mb-1"><b>Produktivitas Regu Parting</b></div>
        <div class="row mb-3">
        <div class="col pr-1">
            <div class="border rounded p-1 mb-2">
                <small>Jam Awal Input</small>
                <div class="font-weight-bold">{{ $produksi['waktu_parting']['awal'] ?? "-" }}</div>
            </div>
        </div>
        <div class="col px-1">
            <div class="border rounded p-1 mb-2">
                <small>Jam Akhir Input</small>
                <div class="font-weight-bold">{{ $produksi['waktu_parting']['akhir'] ?? "-" }}</div>
            </div>
        </div>
        <div class="col pl-1">
            <div class="border rounded p-1 mb-2">
                <small>Total Jam Input</small>
                <div class="font-weight-bold">{{ gmdate("H:i", $produksi['waktu_parting']['jam_kerja'] ?? "0")}} Jam/Menit</div>
            </div>
        </div>
        </div>
        <div class="row mb-3">
            <div class="col pr-1">
                <div class="border rounded p-1 mb-2">
                    ...
                    <div class="font-weight-bold">{{ $produksi['countabf_parting'] ?? "0" }}</div>
                </div>
            </div>
            <div class="col px-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-warning">[EKSPEDISI]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countekspedisi_parting'] ?? "0" }}</div>
                </div>
            </div>
            <div class="col pl-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-warning">[TITIP CS]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countcs_parting'] ?? "0"}}</div>
                </div>
            </div>
            <div class="col pl-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-info">[CHILLER]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countchiller_parting'] ?? "0"}}</div>
                </div>
            </div>
        </div>
    </div>
...
<div id="dashboard-loading-pageSepuluh" style="height: 30px">
