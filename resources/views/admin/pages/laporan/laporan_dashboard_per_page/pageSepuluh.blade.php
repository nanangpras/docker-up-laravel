<div class="row mb-3">
    <div class="col-md-6">
        <div class="font-weight-bold">Summary Bahan Baku Whole Chicken</div>
            ...
            <table class="table default-table">
                ...
                <tbody>
                    @php
                        ...
                        $berat = 0;
                    @endphp
                    @foreach ($produksi['bb_whole'] as $row)
                        @php
                            $ekor += $row->total;
                            $berat += $row->kg;
                        @endphp
                ...
                <tfoot>
                    <tr>
                        <th colspan="3">Total Bahan Baku Whole Chicken</th>
    ...
    <div class="col-md-6">
        <div class="font-weight-bold">Summary Produksi Whole Chicken</div>
        <div class="form-group outer-table-scroll">
            <table class="table default-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    @php
                        ...
                        $berat = 0;
                    @endphp
                    @foreach ($produksi['fg_whole'] as $row)
                        @php
                            $ekor += $row->total;
                            $berat += $row->kg;
                        @endphp
                ...
                <tfoot>
                    <tr>
                        <th>Total Hasil Produksi Whole Chicken</th>
    ...
    <div class="col-md-12">
        <div class="mb-1"><b>Produktivitas Regu Whole Chicken</b></div>
        <div class="row mb-3">
        <div class="col pr-1">
            <div class="border rounded p-1 mb-2">
                <small>Jam Awal Input</small>
                <div class="font-weight-bold">{{ $produksi['waktu_whole']['awal'] ?? "-" }}</div>
            </div>
        </div>
        <div class="col px-1">
            <div class="border rounded p-1 mb-2">
                <small>Jam Akhir Input</small>
                <div class="font-weight-bold">{{ $produksi['waktu_whole']['akhir'] ?? "-" }}</div>
            </div>
        </div>
        <div class="col pl-1">
            <div class="border rounded p-1 mb-2">
                <small>Total Jam Input</small>
                <div class="font-weight-bold">{{ gmdate("H:i", $produksi['waktu_whole']['jam_kerja'] ?? "0")}} Jam/Menit</div>
            </div>
        </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col pr-1">
                <div class="border rounded p-1 mb-2">
                    ...
                    <div class="font-weight-bold">{{ $produksi['countabf_whole'] ?? "0" }}</div>
                </div>
            </div>
            <div class="col px-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-warning">[EKSPEDISI]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countekspedisi_whole'] ?? "0" }}</div>
                </div>
            </div>
            <div class="col pl-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-warning">[TITIP CS]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countcs_whole'] ?? "0"}}</div>
                </div>
            </div>
            <div class="col pl-1">
                <div class="border rounded p-1 mb-2">
                    <small><span class="status status-info">[CHILLER]</span></small>
                    <div class="font-weight-bold">{{ $produksi['countchiller_whole'] ?? "0"}}</div>
                </div>
            </div>
        </div>
    </div>
...
<div id="dashboard-loading-pageSebelas" style="height: 30px">
