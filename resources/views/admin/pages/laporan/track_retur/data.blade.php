            ...
            <thead>
                <tr>
                    <th class="table-success" colspan="3">
                        <div class="float-right">{{ $item->data_order->sales_channel }}</div>
                        {{ $item->data_order->no_so }}
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <b>NS Internal ID :</b> {{ $item->data_order->netsuite_internal_id }}<br>
                        <b>Order ID : </b> <a href="{{ route('salesorder.detail', $item->data_order->id) }}" target="_blank">{{ $item->data_order->id }}</a><br>
                        <b>Tanggal SO : </b> {{ $item->data_order->tanggal_so }}<br>
                        <b>Tanggal Kirim : </b> {{ $item->data_order->tanggal_kirim }}
                    </td>
                    <td>
                        <b>Konsumen :</b> {{ $item->data_order->ordercustomer->nama }}<br>
                        <b>Alamat Kirim :</b>
                        <div style="max-width: 400px" class="status status-warning">{{ $item->data_order->alamat_kirim }}</div>
                    </td>
                    <td>
                        @php
                            $qty        =   0 ;
                            ...
                            $r_berat    =   0 ;
                        @endphp
                        @foreach ($item->data_order->list_order as $list)
                        @php
                            $qty        +=  $list->qty ;
                            $berat      +=  $list->berat ;
