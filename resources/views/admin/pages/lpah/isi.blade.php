...
<div class="row mb-3">
    <div class="col-12">
        <b>{{ $kr_isi }}x Timbang</b><br>
        <b>Total : {{ number_format($isi, 2) }} Kg</b>
    </div>
</div>
    ...
    @foreach ($data as $i => $row)
        <div class="border-bottom p-2">
            ...
            @if ($produksi->lpah_status == 2)
                <button class="btn btn-neutral float-right btn-sm p-0 px-1" data-toggle="modal" data-target="#modal{{ $row->id }}"><i class="fa fa-edit"></i></button>
            @endif
        </div>
        @if ($produksi->lpah_status == 2)
