...
@php $item_qty_akhir        = ($row->qty_saldo_awal + $TotalQtyInbound) - $TotalQtyOutbound  @endphp
@php $item_berat_akhir      = ($row->berat_saldo_awal + $TotalBeratInbound) - $TotalBeratOutbound @endphp
@php $item_krg_akhir        = ($row->karung_saldo_awal + $TotalKarungInbound) - $TotalKarungOutbound  @endphp
...
<button type="button" class="btn btn-blue float-right mb-2 downloadRangeSOH"><i class="fa fa-spinner fa-spin spinerloading" style="display:none;"></i> <span id="text">Download SOH</span></button>
<div id="container">
    <div class="table-responsive table-height">
        <table class="table table-bordered table-striped table-hover table-sticky" border="1">
            <thead>
                <tr>
                    <th class="text-center" colspan="14" rowspan="2">SOH TANGGAL {{ date('d-m-Y', strtotime($tanggal_awal_soh)) }} -  {{ date('d-m-Y', strtotime($tanggal_akhir_soh)) }}</th>
                ...
                <tr style="font-weight: 600;">
                    ...
                    <th>{{ number_format($total_reprodKrg, 2, ',', '.') }}</th>
                    {{-- END MTD OUT --}}
                </tr>
            </thead>
            <tbody>
        ...
        @php $item_krg_akhir        = ($row->karung_saldo_awal + $TotalKarungInbound) - $TotalKarungOutbound  @endphp
                ...
                <th class="stuck">
                    ...
                    <a href="{{ route('warehouse.soh_detail') }}?item={{ $row->nama }}&sub_item={{ $row->sub_item }}&parting={{ $row->parting }}&plastik_group={{ $row->plastik_group }}&customer={{ $row->customer_id }}&sub_pack={{ $row->subpack }}&tanggal_awal_soh={{ $tanggal_awal_soh }}&tanggal_akhir_soh={{$tanggal_akhir_soh}}&gradeItem={{ $row->grade_item }}&gudang={{ $row->gudang_id }}" target="_blank"><span class="fa fa-share"></span></a>
                </th>
                <th class="stuck">
                ...
                <td>
                    <div style="width:100px">{{ App\Models\Product_gudang::modusKarungIsi($row->product_id, $row->plastik_group, $row->sub_item, $tanggal_awal_soh, $row->customer_id, $row->grade_item, $row->gudang_id,$tanggal_akhir_soh) }}</div>
                </td> {{-- ISI --}}
                ...
                {{-- TOTAL INBOUND --}}
                <td class="text-right">{{ number_format($TotalQtyInbound, 2,',', '.') }}</td>
            ...
            <form method="post" action="{{ route('warehouse.soh_ia') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="tanggal" id="form-tanggal" class="form-control" value="{{ $tanggal_awal_soh }}" readonly>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" data-id="" class="btn btn-primary update_ia">Save</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    $(".downloadRangeSOH").click(function(){
        let tanggal_awal_soh        = "{{ $tanggal_awal_soh ?? date('Y-m-d') }}";
        let tanggal_akhir_soh       = "{{ $tanggal_akhir_soh ?? date('Y-m-d') }}";
        let beforeDate              = "{{ $beforeDate ?? date('Y-m-d') }}";
        let item_rangesoh           = "{{ $item_rangesoh ?? NULL }}";
        let cari_rangesoh           = "{{ $cari_rangesoh ?? NULL }}";

        let category_rangesoh       = "{{ $category_rangesoh ?? NULL }}";
        let marinated_rangesoh      = "{{ $marinated_rangesoh ?? NULL }}";
        let itemname_rangesoh       = "{{ $itemname_rangesoh ?? NULL }}";
        let grade_rangesoh          = "{{ $grade_rangesoh ?? NULL }}";
        let customername_rangesoh   = "{{ $customername_rangesoh ?? NULL }}";

        let ordering_rangesoh       = "{{ $ordering_rangesoh ?? NULL }}";
        let order_by_rangesoh       = "{{ $order_by_rangesoh ?? NULL }}";
        ...
        $.ajax({
            ...
            cache   : false,
            data    :{
                'tanggal_awal_soh'      : tanggal_awal_soh,
                'tanggal_akhir_soh'     : tanggal_akhir_soh,
                'beforeDate'            : beforeDate,
                'item_rangesoh'         : item_rangesoh,
                'cari_rangesoh'         : cari_rangesoh,
                'category_rangesoh'     : category_rangesoh,
                'marinated_rangesoh'    : marinated_rangesoh,
                'itemname_rangesoh'     : itemname_rangesoh,
                'grade_rangesoh'        : grade_rangesoh,
                'customername_rangesoh' : customername_rangesoh,
                'ordering_rangesoh'     : ordering_rangesoh,
                'order_by_rangesoh'     : order_by_rangesoh,
                'key'                   : 'unduhrangesoh'

            },
            beforeSend: function() {
                $('.downloadRangeSOH').attr('disabled');
                $(".spinerloading").show();
                $("#text").text('Sedang diunduh ...');
            },
            success: function(data) {
                ...
                setTimeout(() => {
                    ...
                    $(".spinerloading").hide();
                    window.location.href = "{{ route('warehouse_dash.index') }}?tanggal_awal_soh=" + tanggal_awal_soh + "&tanggal_akhir_soh=" +tanggal_akhir_soh+ "&beforeDate=" + beforeDate + "&item_rangesoh=" + item_rangesoh +"&category_rangesoh=" + category_rangesoh +"&marinated_rangesoh=" + marinated_rangesoh +"&itemname_rangesoh=" + itemname_rangesoh
                                                +"&grade_rangesoh=" + grade_rangesoh + "&customer_rangesoh=" + customer_rangesoh +"&ordering_rangesoh=" + ordering_rangesoh + "&order_by_rangesoh=" + order_by_rangesoh + "&cari_rangesoh="+ cari_rangesoh +"&key=unduhrangesoh";
                }, 1000);
            }
        });
    })
