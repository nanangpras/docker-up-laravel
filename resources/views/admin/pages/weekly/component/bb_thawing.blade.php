...
<table class="default-table table">
    ...
    <thead>
        <tr>
            ...
            <th class="text" colspan="2" rowspan="2">Jumlah Thawing</th>
            <th class="text" colspan="8">Hasil Produksi Bahan Baku Thawing</th>
    ...
    <tbody>
        ...
        @foreach ($collection as $row)
            @php
                $qty_thawing    +=  $row['qtythawing'];
                $bb_thawing     +=  $row['bbthawing'];
                $qty_wc         +=  $row['qtywc'];
                $bb_wc          +=  $row['bbwc'];
                $qty_pt         +=  $row['qtypt'];
                $bb_pt          +=  $row['bbpt'];
            ...
            <tr>
                ...
                <td class="text">{{ number_format($row['qtythawing']) }}</td>
                <td class="text">{{ number_format($row['bbthawing']) }}</td>
    ...
    <tfoot>
        <tr>
            ...
            <th class="text">{{ number_format($qty_thawing) }}</th>
            <th class="text">{{ number_format($bb_thawing) }}</th>
...
<form method="post" action="{{route('weekly.export')}}">
    @csrf
    <input name="filename" type="hidden" value="export-weekly-bb-thawing.xls">
    <textarea name="html" style="display: none" id="html-bb-thawing"></textarea>
    <button type="submit" id="export-bb-thawing" class="btn btn-blue">Export</button>
</form>

<script>
    $(document).ready(function(){
        var html  = $('#table-bb-thawing').html();
        $('#html-bb-thawing').val(html);
    })
</script>
